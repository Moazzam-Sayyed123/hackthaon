...
// If you use bootstrap icons, ensure it's included somewhere globally:
...
export default function PriceHistory() {
  ...
  /**
   * Redirect unauthenticated / wrong-role users
   */
  useEffect(() => {
  ...
  /**
   * Redirect if product is missing (e.g., direct URL hit)
   */
  useEffect(() => {
    if (!product) {
  ...
  // Load price history from backend when product becomes available
  useEffect(() => {
    if (!product) return;
    ...
    const loadPriceHistory = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          setPriceHistory([]);
          setStats({ currentPrice: Number(product.price) });
          return;
        }
        ...
        const prices = normalized.map((h) => Number(h.newPrice));
  ...
  // While redirecting or loading product, render nothing
  ...
  return (
    <div className="container mt-5 mb-5">
          ...
          <div className="col-md-3 mb-3">
            <div className="card bg-light shadow-sm">
              <div className="card-body">
                ...
                <h3 className="text-primary">₹{stats.currentPrice.toFixed(2)}</h3>
              </div>
            </div>
          </div>
              ...
              <div className="col-md-3 mb-3">
                <div className="card bg-light shadow-sm">
                  <div className="card-body">
                    ...
                    <h3 className="text-success">₹{stats.minPrice.toFixed(2)}</h3>
                    <small className="text-muted">
                      {(
                        ((stats.currentPrice - stats.minPrice) / stats.currentPrice) *
                        100
                      ).toFixed(1)}
                      % above lowest
                    </small>
              ...
              <div className="col-md-3 mb-3">
                <div className="card bg-light shadow-sm">
                  <div className="card-body">
                    ...
                    <h3 className="text-danger">₹{stats.maxPrice.toFixed(2)}</h3>
                    <small className="text-muted">
                      {(
                        ((stats.maxPrice - stats.currentPrice) / stats.currentPrice) *
                        100
                      ).toFixed(1)}
                      % above current
                    </small>
              ...
              <div className="col-md-3 mb-3">
                <div className="card bg-light shadow-sm">
                  <div className="card-body">
                    ...
                    <h3 className="text-warning">₹{stats.avgPrice.toFixed(2)}</h3>
                  </div>
                </div>
              </div>
            </>
            ...
            <div className="card-body">
              {stats && (
                <div>
                    ...
                    <div className="col-md-6">
                      <h6>
                        ...
                        <span className="text-success fw-bold">
                          ₹{stats.recommendedMin.toFixed(2)}
                        </span>
                      </h6>
                      <small className="text-muted">
                    ...
                    <div className="col-md-6">
                      <h6>
                        ...
                        <span className="text-danger fw-bold">
                          ₹{stats.recommendedMax.toFixed(2)}
                        </span>
                      </h6>
                      <small className="text-muted">
                  ...
                  <button className="btn btn-success w-100" onClick={handleSetAutoOrder}>
      ...
      {/* Price History Table (Last 30 Days) */}
      <div className="row">
        <div className="col-md-12">
          <div className="card shadow-sm">
              ...
              <div className="table-responsive">
                {priceHistory.length === 0 ? (
                  <div className="alert alert-info">
                    No price history available. Showing current product price only.
                  </div>
                ) : (
                  <table className="table table-hover table-sm">
                    ...
                    <tbody>
                      {priceHistory.map((entry, idx) => {
                        const prev = entry.previousPrice != null ? Number(entry.previousPrice) : null;
                        const newP = entry.newPrice != null ? Number(entry.newPrice) : null;
                        const currentPrice = stats?.currentPrice || (newP ?? prev ?? 0);
                        const change = newP != null && currentPrice
                          ? (((newP - currentPrice) / currentPrice) * 100).toFixed(2)
                          : "0.00";
                        const displayDate = entry.recordedAt
                        ...
                        return (
                          <tr
                            key={idx}
                            className={Number(change) < 0 ? "table-success" : "table-danger"}
                          >
                            <td className="fw-bold">{displayDate}</td>
                            <td>{prev != null ? `₹${prev.toFixed(2)}` : "-"}</td>
                            <td>{newP != null ? `₹${newP.toFixed(2)}` : "-"}</td>
                            <td>
                              {Number(change) < 0 ? (
